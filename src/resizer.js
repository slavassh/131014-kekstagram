...
define(function() {
  ...
  Resizer.prototype = {
    ...
    redraw: function() {
      ...
      var integerIterations = Math.round((fullSideLength - this._ctx.lineWidth) / (BEND_STEP * 2));
      var correctStep = fullSideLength / integerIterations / 2;

      var x = (-this._resizeConstraint.side / 2) - this._ctx.lineWidth / 2;
      var y = (-this._resizeConstraint.side / 2) - this._ctx.lineWidth / 2 + correctStep;

      this._ctx.fillStyle = '#ffe753';
      this._ctx.beginPath();
      this._ctx.moveTo(x, y);
      while (x < this._resizeConstraint.side / 2 - this._ctx.lineWidth * 2) {
        this._ctx.lineTo(x += correctStep, y -= correctStep);
        this._ctx.lineTo(x += correctStep, y += correctStep);
      }
      while (y < this._resizeConstraint.side / 2 - this._ctx.lineWidth * 2 - correctStep) {
        this._ctx.lineTo(x -= correctStep, y += correctStep);
        this._ctx.lineTo(x += correctStep, y += correctStep);
      }
      while (x > -(this._resizeConstraint.side / 2 - this._ctx.lineWidth * 2)) {
